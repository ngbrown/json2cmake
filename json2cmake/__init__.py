...
def get_relative_path(include_path, source_dir):
        ...
        for (config, files) in self.targets.items():
            ...
            for file in files:
                if directory is not None:
                    file = get_relative_path(file, directory)
                output.write('    %s\n' % file)
            output.write(')\n')
            ...
            for include in config['includes']:
                if directory is not None:
                    include = get_relative_path(include, directory)
                output.write('    %s\n' % include)
            output.write(')\n')
...
name of the CMake project (default: taken from Git root, or 'autogenerated')
    ...
    if args.outfile is not sys.stdout:
        directory = os.path.dirname(os.path.abspath(args.outfile.name))

    database.write(args.outfile, directory, name=args.name)
